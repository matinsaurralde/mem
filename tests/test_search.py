...
class TestScoring:
    def test_ranks_frequent_commands_higher(self, tmp_mem_dir):
        ...
        for _ in range(5):
            storage.append_command(make_command(command="git status", ts=now, repo="myapp"))
        storage.append_command(make_command(command="git log", ts=now, repo="myapp"))

        results = search.search("git", current_repo="myapp")
    ...
    def test_ranks_recent_commands_higher(self, tmp_mem_dir):
        ...
        storage.append_command(make_command(command="docker build .", ts=old, repo="myapp"))
        storage.append_command(make_command(command="docker compose up", ts=now, repo="myapp"))

        results = search.search("docker", current_repo="myapp")
        commands = [cmd.command for cmd, _ in results]
    ...
    def test_context_boost_for_same_repo(self, tmp_mem_dir):
        ...
        now = int(time.time())
        storage.append_command(make_command(command="make test", ts=now, repo="other-repo"))
        storage.append_command(make_command(command="make test", ts=now, repo="myapp"))

        results = search.search("make", current_repo="myapp")
        # The myapp version should score higher due to context boost
        assert len(results) >= 1
        top_cmd, _ = results[0]
        assert top_cmd.repo == "myapp"
    ...
    def test_deduplication_keeps_highest_score(self, tmp_mem_dir):
        ...
        now = int(time.time())
        storage.append_command(make_command(command="npm run dev", ts=now, repo="myapp"))
        storage.append_command(make_command(command="npm run dev", ts=now - 86400, repo="myapp"))

        results = search.search("npm", current_repo="myapp")
        commands = [cmd.command for cmd, _ in results]
        assert commands.count("npm run dev") == 1

    def test_empty_query_returns_empty(self, tmp_mem_dir):
        """Empty query should return no results."""
        storage.append_command(make_command(command="ls", repo="myapp"))
        results = search.search("", current_repo="myapp")
        assert results == []

    def test_no_matches_returns_empty(self, tmp_mem_dir):
        """Query with no matches returns empty list, not an error."""
        storage.append_command(make_command(command="git status", repo="myapp"))
        results = search.search("nonexistent-tool", current_repo="myapp")
        assert results == []
